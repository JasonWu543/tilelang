...
def dense_warmup_kl_kernel(
    ...
    def main(
        ...
        with T.Kernel(seq_len, heads, threads=threads) as (bx, by):
            ...
            # Fragments for Q (loaded once per query token)
            q_idx_frag = T.alloc_fragment([D_idx], dtype)
            q_mla_frag = T.alloc_fragment([D_mla], dtype)
            q_tail_frag = T.alloc_fragment([D_tail], dtype)
            ...
            # Load query vectors (once per token)
            for d in T.Parallel(D_idx):
                q_idx_frag[d] = Q_indexer[bos + s_i, h_i, d]
            for d in T.Parallel(D_mla):
                q_mla_frag[d] = Q_mla[bos + s_i, h_i, d]
            for d in T.Parallel(D_tail):
                q_tail_frag[d] = Q_mla[bos + s_i, h_i, D_mla + d]
            ...
            for block_i in T.Pipelined(num_blocks, num_stages=num_stages):
                ...
                # Load K blocks
                for bk_i, d_i in T.Parallel(BK, D_idx):
                    k_idx = k_start + bk_i
                    if k_idx < k_end:
                ...
                for bk_i, d_i in T.Parallel(BK, D_mla):
                    k_idx = k_start + bk_i
                    if k_idx < k_end:
                ...
                for bk_i, d_i in T.Parallel(BK, D_tail):
                    k_idx = k_start + bk_i
                    if k_idx < k_end:
                ...
                for bk_i in T.Parallel(BK):
                    for d in T.Parallel(D_idx):
                        score_idx[bk_i] += q_idx_frag[d] * K_idx_shared[bk_i, d]
                
                # MLA attention: Q @ K^T (main + tail)
                for bk_i in T.Parallel(BK):
                    for d in T.Parallel(D_mla):
                        score_mla[bk_i] += q_mla_frag[d] * K_mla_shared[bk_i, d]
                    for d in T.Parallel(D_tail):
                        score_mla[bk_i] += q_tail_frag[d] * K_tail_shared[bk_i, d]
            ...
            for block_i in T.Pipelined(num_blocks, num_stages=num_stages):
                ...
                # Reload K blocks (same as pass 1)
                for bk_i, d_i in T.Parallel(BK, D_idx):
                    k_idx = k_start + bk_i
                    if k_idx < k_end:
                        K_idx_shared[bk_i, d_i] = K_indexer[bos + k_idx, h_i, d_i]
                ...
                for bk_i in T.Parallel(BK):
                    ...
                    if is_valid:
                        score_idx[bk_i] = score_idx[bk_i] * sm_scale_indexer
                        score_mla[bk_i] = score_mla[bk_i] * sm_scale_mla
                        sum_exp_idx[0] += T.exp(score_idx[bk_i] - max_score_idx[0])
                        sum_exp_mla[0] += T.exp(score_mla[bk_i] - max_score_mla[0])
            ...
            for block_i in T.Pipelined(num_blocks, num_stages=num_stages):
                ...
                for bk_i in T.Parallel(BK):
                    ...
                    if is_valid:
                        ...
                        p_idx = T.exp(score_idx[bk_i] - max_score_idx[0]) / sum_exp_idx[0]
                        
                        # log_p = score - max - log(sum_exp)
                        log_p_idx = score_idx[bk_i] - max_score_idx[0] - T.log(sum_exp_idx[0])
                        log_p_mla = score_mla[bk_i] - max_score_mla[0] - T.log(sum_exp_mla[0])
...
def torch_ref_dense_kl(
    ...
    s_main = torch.einsum("shd,Shd->shS", Q_mla[..., :dim_v].float(), K_mla_expanded[..., :dim_v].float()).mul(sm_scale_mla)
    if tail_dim > 0:
        s_tail = torch.einsum("shd,Shd->shS", Q_mla[..., dim_v:].float(), K_mla_expanded[..., dim_v:].float()).mul(sm_scale_mla)
